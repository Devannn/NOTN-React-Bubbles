...
const colors = [
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                // headers.set('Authorization', 'Basic ' + base64.encode('admin:nN38vvDU'));	
                const data = await fetch(`http://${NTOPNG_HOST}:${NTOPNG_PORT}/lua/rest/v2/get/host/l7/stats.lua?ifid=${IFID}&host=192.168.1.143`, {
                    method: 'GET',
                    headers: {
                        'Content-Type': 'application/json',
                        'Authorization': 'Basic ' + base64.encode('admin:nN38vvDU')	
                    },
    ...
    const calculateNewPositions = (currentStats, currentPositions) => {
        const visibleStats = currentStats.filter(stat => stat.value > 0);
        ...
        const totalUsers = visibleStats.reduce((sum, stat) => sum + stat.value, 0);
    ...
    useEffect(() => {
        if (typeof data.rsp !== 'undefined' && data.rsp.length > 0) {
            ...
            const initialPositions = data.rsp.map((stat, index) => {
                const angle = (index / data.rsp.length) * 2 * Math.PI;
                const radius = 150;
                return {
    ...
    useEffect(() => {
        if (positions.length > 0) {
            const newPositions = calculateNewPositions(data.rsp, positions);
    ...
    return typeof data.rsp !== 'undefined' && data.rsp.length > 0 ? (
        <Container>
            <BubblesContainer>
                <AnimatePresence>
                    ...
                    <Bubble
                        ...
                        animate={{
                            scale: 1,
                            width: getBubbleSize(data.rsp.reduce((sum, stat) => sum + stat.value, 0)),
                            height: getBubbleSize(data.rsp.reduce((sum, stat) => sum + stat.value, 0)),
                            x: 0,
                            y: 0,
                        }}
                        transition={{
                    ...
                    >
                        <AppName style={{ fontSize: getFontSize(getBubbleSize(data.rsp.reduce((sum, stat) => sum + stat.value, 0))) }}>
                            Total Packets
                        </AppName>
                        <UserCount style={{ fontSize: getFontSize(getBubbleSize(data.rsp.reduce((sum, stat) => sum + stat.value, 0))) }}>
                            {data.rsp.reduce((sum, stat) => sum + stat.value, 0).toLocaleString()}
                        </UserCount>
                    </Bubble>
                    {/* Other Bubbles */}
                    {!loading && data.rsp
                        ...
                        .map((stat, index) => {
                            const size = getBubbleSize(stat.value);
                            ...
                            return (
                                ...
                                >
                                    <AppName style={{ fontSize: getFontSize(size / 1.5) }}>
                                        {stat.label}
                                    </AppName>
                                    <UserCount style={{ fontSize: getFontSize(size) * 0.8 }}>
                                        {data.rsp[index].value.toLocaleString()} packets
                                    </UserCount>
                                </Bubble>
                            );
                        })}
